...
extern "C"
{
    ...
    EXPORT wxImage* wxImage_CreateFromDataEx(int width, int height, wxUint8* data,
                                             bool isStaticData)
    {
        return new wxImage(width, height, data, isStaticData);
    }

    EXPORT void wxImage_Delete(wxImage* image)
    {
        delete image;
    }
    ...
    EXPORT int wxColour_GetInt(wxColour* colour)
    {
        int r = colour->Red();
        int g = colour->Green();
        int b = colour->Blue();
    ...
    EXPORT wxColour* wxColour_CreateFromUnsignedInt(unsigned int rgba)
    {
        return new wxColour((rgba >> 24) & 0xFF, (rgba >> 16) & 0xFF, (rgba >> 8) & 0xFF,
                            rgba & 0xFF);
    }

    EXPORT unsigned int wxColour_GetUnsignedInt(wxColour* colour)
    {
        int r = colour->Red();
        int g = colour->Green();
        int b = colour->Blue();
